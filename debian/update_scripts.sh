...
SCRIPTS=(
    ...
    check_config.sh update_ui.sh delaytest.sh update_config.sh setup.sh
    ufw.sh kernel.sh optimize.sh menu.sh
)
...
download_verified() {
    ...
    bash -n "$TMP_DIR/$name"
    if head -n1 "$TMP_DIR/$name" | grep -q '^#!/bin/sh'; then sh -n "$TMP_DIR/$name"; fi
}

echo -e "${CYAN}正在下载并验证管理脚本（固定提交 $BASE_REF）...${NC}"
...
echo -e "${GREEN}全部管理脚本已完成固定版本下载、语法校验和事务式更新。${NC}"
