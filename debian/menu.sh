...
run_script() {
    ...
    if [[ "$quiet" == --quiet ]]; then bash "$SCRIPT_DIR/$script_name" >/dev/null; else bash "$SCRIPT_DIR/$script_name"; fi
}
run_systemctl() {
...
confirm_yes() {
    ...
    while true; do
        ...
        case "$answer" in [Yy]) return 0;; [Nn]) return 1;; *) echo -e "${YELLOW}请输入 y 或 n。${NC}";; esac
    done
}
uninstall_sbshell() {
    ...
    if [ -f /etc/sing-box/scripts/menu.sh ]; then sed -i '/# sing-box 快捷方式/,/alias sb=/d' /root/.bashrc 2>/dev/null || true; fi
    rm -rf "$SCRIPT_DIR"
...
download_all_scripts() {
    ...
    for script in "${SCRIPTS[@]}"; do
        ...
        if head -n1 "$tmpdir/$script" | grep -q '^#!/bin/sh'; then sh -n "$tmpdir/$script" || return 1; fi
    done
    for script in "${SCRIPTS[@]}"; do [ -f "$SCRIPT_DIR/$script" ] && cp -a "$SCRIPT_DIR/$script" "$backupdir/$script"; done
    restore() { local item; for item in "${SCRIPTS[@]}"; do if [ -f "$backupdir/$item" ]; then install -o root -g root -m 0755 "$backupdir/$item" "$SCRIPT_DIR/$item"; else rm -f "$SCRIPT_DIR/$item"; fi; done; }
    for script in "${SCRIPTS[@]}"; do install -o root -g root -m 0755 "$tmpdir/$script" "$SCRIPT_DIR/$script" || { restore; return 1; }; done
}
check_and_download_scripts() { local script; for script in "${SCRIPTS[@]}"; do if [ ! -s "$SCRIPT_DIR/$script" ]; then echo -e "${YELLOW}发现缺失脚本: $script${NC}"; download_all_scripts || return 1; return 0; fi; done; }
...
run_initialization() { select_role; echo -e "${YELLOW}为 '$ROLE' 角色进行首次初始化。${NC}"; read -rp "按回车开始，或输入 'skip' 仅下载脚本进入菜单: " init_choice; if [[ "$init_choice" =~ ^[Ss]kip$ ]]; then download_all_scripts || return 1; elif [ "$ROLE" = server ]; then server_initialize || return 1; else client_initialize || return 1; fi; touch "$INITIALIZED_FILE"; chmod 0644 "$INITIALIZED_FILE"; }
...
handle_server_choice() { local choice; read -rp '请选择操作: ' choice; case "$choice" in 1) run_systemctl 启动sing-box start;; 2) run_systemctl 停止sing-box stop;; 3) run_systemctl 重启sing-box restart;; 4) run_systemctl 设置开机自启 enable;; 5) journalctl -u sing-box --output cat -f;; 6) run_script 更新服务端配置文件 update_config.sh;; 7) if command -v sing-box >/dev/null 2>&1; then run_script 检查\ sing-box\ 更新 check_update.sh; else run_script 安装/更新\ sing-box install_singbox.sh; fi;; 8) run_script 更新脚本 update_scripts.sh;; 9) run_script 证书申请 setup.sh;; 10) run_script 更换XanMod内核 kernel.sh;; 11) run_script 网络优化 optimize.sh;; 12) run_script 手动配置防火墙 ufw.sh;; 13) uninstall_sbshell;; 0) exit 0;; *) echo -e "${RED}无效的选择。${NC}";; esac; }
main() { require_cmd curl; install -d -o root -g root -m 0755 "$SCRIPT_DIR"; cd "$SCRIPT_DIR"; [ "${1:-}" = menu ] && shift || true; if [ ! -f "$INITIALIZED_FILE" ]; then run_initialization || exit 1; else if [ -f "$ROLE_FILE" ]; then ROLE=$(cat "$ROLE_FILE"); else rm -f "$INITIALIZED_FILE"; run_initialization || exit 1; fi; check_and_download_scripts || exit 1; fi; case "$ROLE" in client) setup_alias; while true; do show_client_menu; handle_client_choice; done;; server) setup_alias; while true; do show_server_menu; handle_server_choice; done;; *) echo -e "${RED}角色无效: $ROLE${NC}" >&2; exit 1;; esac; }
main "$@"
