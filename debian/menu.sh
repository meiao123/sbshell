...
SCRIPTS=(
  ...
  manage_autostart.sh check_config.sh check_environment.sh set_network.sh clean_nft.sh kernel.sh optimize.sh set_defaults.sh
...
download_all_scripts() {
    ...
    backupdir=$(mktemp -d /tmp/sbshell-backup.XXXXXX)
    ...
    restore() {
        ...
        for item in "${SCRIPTS[@]}"; do
            if [ -f "$backupdir/$item" ]; then install -o root -g root -m 0755 "$backupdir/$item" "$SCRIPT_DIR/$item"; else rm -f "$SCRIPT_DIR/$item"; fi
        done
    }
    for script in "${SCRIPTS[@]}"; do
...
prepare_scripts() {
    ...
    download_all_scripts || { echo -e "${RED}脚本更新失败，保留现有安装。${NC}" >&2; return 1; }
}

client_initialize() {
    prepare_scripts || return 1
    run_script '检查系统环境' check_environment.sh || return 1
    run_script '安装/更新 sing-box' install_singbox.sh || return 1
    run_script '配置代理模式' switch_mode.sh || return 1
    run_script '配置订阅链接' manual_input.sh || return 1
    run_script '启动 sing-box' start_singbox.sh || return 1
...
select_role() {
    ...
    case "$role_choice" in 1) ROLE=client;; 2) ROLE=server;; *) ROLE=client; echo -e "${YELLOW}无效选择，默认客户端。${NC}";; esac
    printf '%s\n' "$ROLE" > "$ROLE_FILE"
...
run_initialization() {
    ...
    if [[ "$init_choice" =~ ^[Ss]kip$ ]]; then download_all_scripts || return 1
    elif [ "$ROLE" = server ]; then server_initialize || return 1
    else client_initialize || return 1
    fi
    touch "$INITIALIZED_FILE"; chmod 0644 "$INITIALIZED_FILE"
}
setup_alias() {
    local bashrc=${HOME:-/root}/.bashrc
    if ! grep -Fq 'alias sb=' "$bashrc" 2>/dev/null; then printf '\n# sing-box 快捷方式\nalias sb='"'bash /etc/sing-box/scripts/menu.sh'"'\n' >> "$bashrc"; fi
...
handle_client_choice() {
    ...
    case "$choice" in
      1) run_script '配置代理模式' switch_mode.sh && run_script '配置订阅链接' manual_input.sh && run_script '启动 sing-box' start_singbox.sh;;
...
main() {
    ...
    cd "$SCRIPT_DIR"
    [ "${1:-}" = menu ] && shift || true
    if [ ! -f "$INITIALIZED_FILE" ]; then run_initialization || exit 1
    else
        if [ -f "$ROLE_FILE" ]; then ROLE=$(cat "$ROLE_FILE"); else rm -f "$INITIALIZED_FILE"; run_initialization || exit 1; fi
        check_and_download_scripts || exit 1
    fi
    case "$ROLE" in
      client) setup_alias; while true; do show_client_menu; handle_client_choice; done;;
      server) setup_alias; while true; do show_server_menu; handle_server_choice; done;;
