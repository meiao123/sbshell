...
valid_ip() { [[ "$1" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]] && awk -F. '{for(i=1;i<=4;i++) if($i>255) exit 1}' <<< "$1"; }
valid_dns() { valid_ip "$1" || [[ "$1" =~ ^[0-9A-Fa-f:]+$ ]]; }
...
while true; do
    ...
    if ! [[ "$IP_ADDRESS" =~ ^[^/]+/[0-9]{1,2}$ ]]; then
        echo -e "${RED}IP 必须使用 CIDR 格式，例如 192.168.1.10/24。${NC}"; continue
    fi
    IP_ONLY=${IP_ADDRESS%/*}; PREFIX=${IP_ADDRESS#*/}
    valid_ip "$IP_ONLY" || { echo -e "${RED}IP 地址无效。${NC}"; continue; }
    [ "$PREFIX" -le 32 ] || { echo -e "${RED}CIDR 前缀无效。${NC}"; continue; }
    valid_ip "$GATEWAY" || { echo -e "${RED}IPv4 网关地址无效。${NC}"; continue; }
...
iface $INTERFACE inet static
    ...
    netmask $(python3 - <<PY
import ipaddress
print(ipaddress.ip_network('0.0.0.0/$PREFIX').netmask)
PY
)
    gateway $GATEWAY
EOL
