...
require_service() { systemctl list-unit-files --type=service --no-legend "$1.service" 2>/dev/null | grep -q .; }
...
[ ! -L /etc/resolv.conf ] || { echo -e "${RED}/etc/resolv.conf 是符号链接，已拒绝覆盖。${NC}" >&2; exit 1; }
...
if grep -Eq "^[[:space:]]*iface[[:space:]]+$INTERFACE([[:space:]]|$)" /etc/network/interfaces; then
    echo -e "${RED}目标接口 $INTERFACE 已存在 iface 配置，本脚本拒绝覆盖其原有配置。${NC}" >&2
    exit 1
fi
if grep -Eq '^[[:space:]]*(source|source-directory)[[:space:]]' /etc/network/interfaces; then
    echo -e "${RED}/etc/network/interfaces 包含 source/source-directory，本脚本拒绝覆盖。${NC}" >&2
    exit 1
fi
...
while true; do
    ...
    [[ "$IP_ADDRESS" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}$ ]] || { echo -e "${RED}IPv4 必须使用 CIDR 格式。${NC}"; continue; }
...
# Sbshell managed static network
...
EOL
    ...
    mv -f "$tmp_interfaces" /etc/network/interfaces
    ...
    echo -e "${RED}网络配置未通过验证，正在恢复原配置。${NC}" >&2
