...
NFT_TMP=$(mktemp /tmp/sbshell-tproxy.XXXXXX); OLD_TABLE=$(mktemp /tmp/sbshell-tproxy-table.XXXXXX); STATE_FILE=/etc/sing-box/tproxy.state
trap 'rm -f "$NFT_TMP" "$OLD_TABLE"' EXIT
cat > "$NFT_TMP" <<EOF
table inet sing-box {
...
rollback() {
    ...
    [ ! -s "$OLD_TABLE" ] || { nft list table inet sing-box >/dev/null 2>&1 && nft delete table inet sing-box || true; nft -f "$OLD_TABLE" 2>/dev/null || true; }
}
