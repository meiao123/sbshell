...
rollback() {
    ...
    # 重新应用时旧的 rule/route 已在前面被删除。若本次应用失败，必须把它们放回去，
    # 否则 nft 表虽然恢复了，TProxy 仍然是坏的，而 state 文件依旧声称
    # RULE_OWNED=1/ROUTE_OWNED=1（state 与事实不符）。
...
[ "$prior_rule_created" -eq 1 ] && RULE_CREATED=0
[ "$prior_route_created" -eq 1 ] && [ "$ROUTE_CREATED" -eq 0 ] && ROUTE_CREATED=0
cat > "$STATE_FILE" <<EOF
OWNER=sbshell
MODE=TProxy
TUN_TABLE_CREATED=0
