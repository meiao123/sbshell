...
MODE=$(sed -n 's/^MODE=//p' /etc/sing-box/mode.conf | head -n1)
[ "$MODE" = TProxy ] || exit 0
...
TMP=$(mktemp /tmp/sbshell-nft.XXXXXX); trap 'rm -f "$TMP"' EXIT
cat > "$TMP" <<EOF
table inet sing-box {
...
EOF
nft -c -f "$TMP"
OLD=$(mktemp /tmp/sbshell-ruleset.XXXXXX); trap 'rm -f "$TMP" "$OLD"' EXIT
nft list ruleset > "$OLD" 2>/dev/null || :
