...
SCRIPTS=(check_environment.sh install_singbox.sh manual_input.sh manual_update.sh auto_update.sh configure_tproxy.sh configure_tun.sh start_singbox.sh stop_singbox.sh clean_nft.sh set_defaults.sh commands.sh switch_mode.sh manage_autostart.sh check_config.sh update_ui.sh menu.sh)
for script in "${SCRIPTS[@]}"; do curl --fail --silent --show-error --location --proto '=https' --tlsv1.2 --connect-timeout 10 --max-time 60 "$BASE_URL/$script" -o "$TMP_DIR/$script"; [ -s "$TMP_DIR/$script" ] || exit 1; bash -n "$TMP_DIR/$script"; if head -n1 "$TMP_DIR/$script" | grep -q '^#!/bin/sh'; then sh -n "$TMP_DIR/$script"; fi; done
...
echo 'OpenWrt 管理脚本已完成固定发布提交下载、语法校验和事务式更新。'
